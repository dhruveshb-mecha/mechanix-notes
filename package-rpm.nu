...
# RPM Packaging Script for Mechanix Notes
# Usage: nu package-rpm.nu notes <output-dir>
...
def resolve-build-directory [app_folder: path, pkg_arch: string] {
    ...
    let arch_candidates = if ($pkg_arch == "x86_64" or $pkg_arch == "x64") {
        ["x64", "x86_64"]
    } else if ($pkg_arch == "aarch64" or $pkg_arch == "arm64") {
        ["aarch64", "arm64"]
    } else {
        [$pkg_arch]
    }
    ...
    let candidates = (
        $arch_candidates
        | each { |arch| ($folder | path join "build" "elinux" $arch "release" "bundle") }
        | append ($folder | path join "build" "elinux" "bundle")
    )
...
def generate-spec [pkg_name: string, version: string, release: string, arch: string, app: record, pubspec: record, artifacts: record] {
    ...
    let base_lines = [
        ...
        "License: BSD"
        $"Packager: ($maintainer)"
        "URL: https://github.com/mineshp-mecha/mechanix-notes"
        $"BuildArch: ($arch)"
        "AutoReqProv: no"
    ]
